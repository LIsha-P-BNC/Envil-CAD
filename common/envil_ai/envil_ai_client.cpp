...
static const char* ENVIL_DEFAULT_SYSTEM_PROMPT =
        ...
        "- To tie several pins onto one rail (VIN, a switch node, GND), the cleanest way is a "
        "net LABEL on each pin (add_label with the same name) rather than one long wire "
        "threading through them. Labels of the same name are connected.\n"
