...
PANEL_COMMON_SETTINGS_BASE::PANEL_COMMON_SETTINGS_BASE( wxWindow* parent, wxWindowID id, const wxPoint& pos, const wxSize& size, long style, const wxString& name ) : RESETTABLE_PANEL( parent, id, pos, size, style, name )
{
	...
	m_rbFallback->SetToolTip( _("Software graphics (for computers which do not support Anvil's hardware acceleration requirements)") );
	...
	m_disableCustomCursors->SetToolTip( _("When enabled, Anvil will use default system cursors instead of custom ones") );
	...
	m_rbIconSizeNormal->SetToolTip( _("Use the default Anvil icon size in the toolbars") );
