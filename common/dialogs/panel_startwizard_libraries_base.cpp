...
PANEL_STARTWIZARD_LIBRARIES_BASE::PANEL_STARTWIZARD_LIBRARIES_BASE( wxWindow* parent, wxWindowID id, const wxPoint& pos, const wxSize& size, long style, const wxString& name ) : wxPanel( parent, id, pos, size, style, name )
{
	...
	m_stIntro = new wxStaticText( this, wxID_ANY, _("Anvil comes with a large set of symbol and footprint libraries maintained by the Anvil librarian team.  You may also create your own libraries, and install third-party ones from the Plugin and Content Manager or other sources.\n\nLibrary tables are the configuration files which list the libraries to be loaded. Global libraries are available in every project, and you may also add project-specific libraries if desired."), wxDefaultPosition, wxDefaultSize, 0 );
	m_stIntro->Wrap( -1 );
	...
	m_stQuery = new wxStaticText( this, wxID_ANY, _("How would you like Anvil to create them?"), wxDefaultPosition, wxDefaultSize, 0 );
	m_stQuery->Wrap( 400 );
	bSizer8->Add( m_stQuery, 0, wxALL, 5 );

	m_rbDefaultTables = new wxRadioButton( this, wxID_ANY, _("Start with the built-in Anvil libraries (recommended)"), wxDefaultPosition, wxDefaultSize, 0 );
	...
	m_rbImport->SetToolTip( _("Library tables will be imported from the version you selected on the previous step.  Any tables that cannot be imported will be created with the default Anvil libraries.") );
	...
	m_stWarning = new wxStaticText( this, wxID_ANY, _("The built-in library tables could not be found in the expected location. This usually means that you have installed the Anvil software without also installing the libraries. You can proceed with setup, but you will need to install the libraries separately in order for them to be available."), wxDefaultPosition, wxDefaultSize, 0 );
	m_stWarning->Wrap( -1 );
