...
DIALOG_IMPORTED_LAYERS_BASE::DIALOG_IMPORTED_LAYERS_BASE( wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos, const wxSize& size, long style ) : DIALOG_SHIM( parent, id, title, pos, size, style )
{
	...
	m_cbKeepKiCadLayerNames = new wxCheckBox( this, wxID_ANY, _("Keep Anvil layer names"), wxDefaultPosition, wxDefaultSize, 0 );
	m_cbKeepKiCadLayerNames->SetToolTip( _("If checked, layers will keep their standard Anvil names instead of being renamed to the imported layer names.") );
